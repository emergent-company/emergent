...
func (r *Repository) Create(ctx context.Context, tx bun.Tx, project *Project) error {
	_, err := tx.NewInsert().
		Model(project).
		Returning("id, organization_id, name, project_info, chat_prompt_template, auto_extract_objects, auto_extract_config, created_at, updated_at, auto_merge_extraction_branches").
		Exec(ctx)
