...
// passthrough) and that the response echoes the applied values back.
func TestListPacksPaginationHandler(t *testing.T) {
	...
	// DESC ordering by updated_at: alpha (newest), global, beta (oldest).
	orderedIDs := []string{alphaID, globalID, betaID}
	...
	t.Run("offset passthrough pages the list", func(t *testing.T) {
		...
		require.Len(t, resp.Schemas, 1)
		assert.Equal(t, globalID, resp.Schemas[0].ID)
		assert.Equal(t, 3, resp.Total)
	})
	...
	t.Run("full catalog keeps updated_at desc order", func(t *testing.T) {
		resp := decode(t, getListPacks(t, h, projectID, ""))
		assert.Equal(t, names(resp), []string{"alpha-pack", "global-pack", "beta-pack"})
...
// project-owned + global rows, updated_at DESC ordering, search, pagination,
// count/list consistency, and the MCP-parity wire shape (visibility always
// emitted; project_id/org_id omitted for global rows).
func TestListSchemaPacksRepository(t *testing.T) {
	...
	// updated_at (newest first): alpha(40) gamma(30) beta(20) delta(10) zed(0)
	aID := insertCatalogSchema(t, ctx, db, "alpha", desc("photo album"), &projectID, timePtrOffset(t, base, 40))
	gID := insertCatalogSchema(t, ctx, db, "gamma", nil, nil, timePtrOffset(t, base, 30)) // global, NULL description
	...
	// Helper to detect a NULL description scan failure: if bun cannot scan a
	// NULL description into the string field this test already fails above.

	t.Run("project catalog includes owned and global rows in desc order", func(t *testing.T) {
		...
		assert.Equal(t, []string{aID, gID, bID, dID, zID}, got)
	})
	...
	t.Run("pagination pages full ordered set while total stays constant", func(t *testing.T) {
		...
		assert.Equal(t, []string{aID, gID}, []string{page1[0].ID, page1[1].ID})

		page2, total2, err := repo.ListSchemaPacks(ctx, projectID, "", 2, 2)
		require.NoError(t, err)
		assert.Equal(t, 5, total2)
		require.Len(t, page2, 2)
		assert.Equal(t, []string{bID, dID}, []string{page2[0].ID, page2[1].ID})

		page3, _, err := repo.ListSchemaPacks(ctx, projectID, "", 2, 4)
		require.NoError(t, err)
		require.Len(t, page3, 1)
		assert.Equal(t, zID, page3[0].ID)
	})
	...
	t.Run("wire shape mirrors MCP SchemaInfo", func(t *testing.T) {
		...
		var global, owned map[string]any
		...
		require.NotNil(t, global, "expected global row in payload")
		...
		// project_id/org_id omitted for the global row.
		_, hasPID := global["project_id"]
		_, hasOID := global["org_id"]
		assert.False(t, hasPID, "global row should omit project_id")
		assert.False(t, hasOID, "global row should omit org_id")
		// NULL description scans to "" and is still emitted (no omitempty).
		assert.Equal(t, "", global["description"])
