...
type mockStoreForProvisioning struct {
	workspaces  map[string]*AgentSandbox
	mu          sync.Mutex
	activeCount int
	activeErr   error
	createErr   error
...
func newMockStore() *mockStoreForProvisioning {
	return &mockStoreForProvisioning{
		workspaces: make(map[string]*AgentSandbox),
	}
}
...
func TestIntegration_AutoProvision_NilConfig(t *testing.T) {
	...
	result, err := ap.ProvisionForSession(t.Context(), "agent-def-1", "", nil, nil, "", nil)
	assert.NoError(t, err)
	assert.Nil(t, result)
}
...
func TestIntegration_AutoProvision_NoProviders_DegradedMode(t *testing.T) {
	...
	ap := NewAutoProvisioner(svc, o, nil, nil, nil, testLogger(), nil)
	...
	result, err := ap.ProvisionForSession(t.Context(), "agent-def-1", "", config, nil, "", nil)
...
func TestIntegration_WorkspaceConfig_RoundTrip(t *testing.T) {
	// Create a config → ToMap → ParseAgentSandboxConfig → verify equality.
	original := &AgentSandboxConfig{
		Enabled: true,
		RepoSource: &RepoSourceConfig{
			Type:   RepoSourceFixed,
			URL:    "https://github.com/org/repo",
	...
	parsed, err := ParseAgentSandboxConfig(m)
	require.NoError(t, err)
...
func TestIntegration_AutoProvision_GetProviderForWorkspace(t *testing.T) {
	...
	ap := NewAutoProvisioner(nil, o, nil, nil, nil, testLogger(), nil)

	t.Run("valid workspace", func(t *testing.T) {
		ws := &AgentSandbox{Provider: ProviderGVisor}
	...
	t.Run("unregistered provider", func(t *testing.T) {
		ws := &AgentSandbox{Provider: ProviderFirecracker}
...
func TestIntegration_AutoProvision_FullPipeline_FixedRepo(t *testing.T) {
	...
	// Step 1: Parse config
	cfg, err := ParseAgentSandboxConfig(config)
	require.NoError(t, err)
	...
	provider, providerType, err := o.SelectProvider(ContainerTypeAgentSandbox, DeploymentSelfHosted, "")
	require.NoError(t, err)
...
func TestIntegration_AutoProvision_FullPipeline_NoRepo(t *testing.T) {
	...
	provider, _, err := o.SelectProvider(ContainerTypeAgentSandbox, DeploymentSelfHosted, "")
	require.NoError(t, err)
...
func TestE2E_AgentType_WorkspaceConfig_SessionFlow(t *testing.T) {
	...
	cfg, err := ParseAgentSandboxConfig(agentDefWorkspaceConfig)
	require.NoError(t, err)
	require.NotNil(t, cfg)
...
func TestIntegration_DefaultAgentSandboxConfig(t *testing.T) {
